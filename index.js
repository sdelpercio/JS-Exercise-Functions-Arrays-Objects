 ...
 * }
*/
function makePersonObject(idHere, nameHere, emailHere,) {
  const myObject = {
    id: idHere,
    name: nameHere,
    email: emailHere
...
function getCarInfoByIndex(inventory, index) {
  const findIndex = inventory.find((item, index) => {
    return index === index;
  })
  return `The car is a ${findIndex.car_make} ${findIndex.car_model}`;
}
...
function getLastCarInfo(carArray) {
  const last = carArray[carArray.length - 1];
 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(carArray, index) {
  const any = carArray[index - 1];
  
  return `This is a ${any.car_make} ${any.car_model}`;
}
...
function sortCarInventory(carArray) {
  const sortedInventory = carArray.sort(function (a, b) {
...
function getModelYears(carArray) {
  ...
  for (let i = 0; i < carArray.length; i++) {
    ...
    allYears.push(currentCar.car_year);
  }
...
function carMaker(odometerValue) {
  const updated = {
    ...
    drive: function(distance) {
      updated.odometer = updated.odometer + distance;
      return updated.odometer;
    }
  };
